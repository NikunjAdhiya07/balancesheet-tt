...
export async function GET(
  ...
  const row = await getIncomeById(Number(id));
...
export async function PUT(
  ...
  await recordAudit("income", id, "update", { before: existing, after: updated });
...
export async function DELETE(
  ...
  const supabase = getSupabase();
  const now = new Date().toISOString();
  const { error } = await supabase.from("income").update({ deleted_at: now }).eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await recordAudit("income", id, "delete", existing);
